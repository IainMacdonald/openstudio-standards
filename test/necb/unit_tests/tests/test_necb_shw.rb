...
class NECB_SHW_Tests < Minitest::Test
  ...
  def test_add_shw_test_output_info
    ...
    test_cases_hash = { :Vintage => ["NECB2011", "BTAPPRE1980", "BTAP1980TO2010"],
                        :FuelType => ["NaturalGas"],
                        :TestCase => ["case-1"],
                        :TestPars => { :epw_file => "CAN_AB_Calgary.Intl.AP.718770_CWEC2016.epw" } }
  ...
  def do_test_add_shw_test_output_info(test_pars:, test_case:)
    # Debug.
    ...
    model.getWaterHeaterMixeds.sort.each do |obj|
      standard.water_heater_mixed_apply_efficiency(obj)
    end
